...
for (i in seq_along(lambda)) {
  ...
  pars <- list(cycle_width =cycle_width_values,
               ...
               K =267800,
               lambda = lamb, # lambda stored in "lamb" coz' I wanna print the progress
  ...
  print(paste("Processing lambda =", lamb))
}

# How to access the result?
# How about save it to *csvs???
write.csv(E_mtx, file="Output_E_lambda_Angambiae.csv", row.names =F)
write.csv(L_mtx, file="Output_L_lambda_Angambiae.csv", row.names =F)
write.csv(N_mtx, file="Output_N_lambda_Angambiae.csv", row.names =F)
write.csv(S_v_mtx, file="Output_S_v_lambda_Angambiae.csv", row.names =F)
write.csv(E_v_mtx, file="Output_E_v_lambda_Angambiae.csv", row.names =F)
write.csv(I_v_mtx, file="Output_I_v_lambda_Angambiae.csv", row.names =F)
write.csv(V_mtx, file="Output_V_lambda_Angambiae.csv", row.names =F)
write.csv(Prev_mtx, file="Output_Prev_lambda_Angambiae.csv", row.names =F)
write.csv(Pos_mtx, file="Output_Pos_lambda_Angambiae.csv", row.names =F)
